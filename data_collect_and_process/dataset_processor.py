...
def process_dataset():
    ...
    print(f"[*] Optimization & Cropping Pipeline Initialized.")
    print(f"[*] Processing DB inside: {SRC_DATASET_DIR}")
    print(f"[*] Target Hardware Profile: OpenMV N6 ({TARGET_SIZE[0]}x{TARGET_SIZE[1]}, JPEG Quality: 75%)\n")
    ...
    for class_name in os.listdir(SRC_DATASET_DIR):
        ...
        for tier_name in os.listdir(src_class_folder):
            ...
            if not os.path.isdir(src_tier_folder) or tier_name.startswith('.') or tier_name.endswith('_FT'):
                continue

            dst_tier_folder = os.path.join(src_class_folder, f"{tier_name}_FT")
            os.makedirs(dst_tier_folder, exist_ok=True)

            print(f" [->] Processing '{tier_name}' -> Creating '{tier_name}_FT'...")
            ...
            for file_name in image_files:
                ...
                try:
                    ...
                    if bird_boxes:
                        ...
                        final_img.save(dst_file_path, 'JPEG', quality=75)
                        success_count += 1
                    else:
            ...
            print(f" [V] Completed {tier_name}_FT: {success_count} smart crops, {fallback_count} center fallbacks.")
